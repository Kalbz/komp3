...
public:
    ...
    string execute(SymbolTable &symbolTable) override
    {
        ...
        if (children.size() >= 3)
        {
            ...
            symbolTable.enterScope(methodIdentifier, "Method", methodType);
            ...
            Scope* methodScope = symbolTable.getCurrentScope();
            Scope* classScope = methodScope->getParentScope();
            if (classScope != nullptr) {
                Record thisRecord("this", classScope->getName());
                symbolTable.put("this", thisRecord);
            }
    ...
    string performSemanticAnalysis(SymbolTable& symbolTable) override {
        symbolTable.enterScope(children[1]->value, "Method", children[0]->value);
...
public:
    ...
    string execute(SymbolTable &symbolTable) override
    {
        ...
        else if (children.size() == 3)
        {
            ...
            symbolTable.put(classIdentifier, classObj);
            symbolTable.enterScope(classIdentifier, "Class", classExtends);
            std::cout << "ClassDeclaration encountered: Identifier=" << classIdentifier << std::endl;
            children[1]->execute(symbolTable);
            children[2]->execute(symbolTable);
            symbolTable.exitScope();
...
class Return : public Node{
    ...
    public:
        ...
        string performSemanticAnalysis(SymbolTable& symbolTable) override {
            ...
            std::string methodType = symbolTable.getCurrentScope()->getDeclaredType();
