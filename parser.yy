...
%token <std::string> PLUSOP MINUSOP MULTOP INT LP RP CLASS PUBLIC STATIC 
%token <std::string> VOID INT_KEYWORD BOOLEAN IF ELSE WHILE RETURN TRUE FALSE THIS NEW LENGTH PRINTLN 
%token <std::string> IDENTIFIER LCB RCB LB RB ASSIGNOP SEMICOLON COMMA DOT ANDOP EQOP 
%token <std::string> OROP DIVOP NOTOP LCHEV RCHEV STRING_KEYWORD MAIN
